...
assert(article.includes('<html lang="ru">'));
assert(article.includes('class="article-translation" lang="en"'));
assert(article.includes("10.1126/sciadv.adz5561"));
assert(article.includes("FINISH: unreviewed and undocumented is unfinished"));
const project = await readFile(join(root, knownRoutes[5], "index.html"), "utf8");
assert.equal((project.match(/<figure class="editorial-figure"/g) ?? []).length, 7);
